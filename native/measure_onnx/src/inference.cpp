...
std::vector<Detection> Inference::runInference(const cv::Mat &input) {
  ...
  bool yolov8 = false;
  // yolo_nas has an output of shape output0:(batchSize, 8400, 80) output1:
  // (batchSize, 8400, 4) yolov8 and yolo11 has an output of shape (batchSize,
  ...
  for (int i = 0; i < rows; ++i) {
    ...
    } else // yolo_nas
    {
      ...
      float *classes_scores =
          reinterpret_cast<float *>(outputs[0].data) + i * 80;
