...
std::vector<std::string> utils::getClassNames(const std::string &path) {
  ...
  std::ifstream f(path);
  if (!f) {
    std::cerr << "[ERROR] Could not open class names file: " << path
              << std::endl;
    return classNames;
  }
  std::string line;
...
BoundingBox utils::scaleCoords(const cv::Size &letterboxShape,
  ...
  int padW = static_cast<int>(std::round(
      ((float)letterboxShape.width - (float)originalShape.width * gain) / 2.f));
  int padH = static_cast<int>(std::round(
      ((float)letterboxShape.height - (float)originalShape.height * gain) /
      2.f));
