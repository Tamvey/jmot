...
Detector::postprocess(const cv::Size &origSize, const cv::Size &letterboxSize,
  ...
  // Extract outputs
  const float *output0_ptr = reinterpret_cast<const float *>(outputs[0].data());
  // const float *output1_ptr = reinterpret_cast<const float
  // *>(outputs[1].data());
...
#if TRT == 10
  auto &dims0 = tensorrt_base_->output_[0].dims_;
  // auto &dims1 = ensorrt_base_->output_[1].dims_;;
#else
  ...
  for (int i = 0; i < numBoxes; ++i) {
    ...
    detection::BoundingBox box{static_cast<int>(std::round(xc - w / 2.0f)),
